...
// Interactive Onboarding Checklist Steps -- derived from real account state
...
// Seller Tiers, Achievements & Instagram Story sharing -- commented out for now
// (re-enable by uncommenting this block and the matching template sections)
/*
const sellerTiers = [
...
// Metrics & Analytics -- derived from real orders/conversations
...
<template>
  <div class="p-4 md:p-6 space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
        ...
        <NuxtLink to="/inbox" class="p-5 rounded-xl border border-default bg-background hover:border-primary transition-all space-y-2 group">
          <div class="flex items-center justify-between">
            <UIcon name="i-simple-icons-instagram" class="size-6 text-pink-500" />
            <UBadge color="primary" variant="subtle" size="xs">{{ metrics.activeDms }} Active DMs</UBadge>
          </div>
        ...
        <NuxtLink to="/orders" class="p-5 rounded-xl border border-default bg-background hover:border-primary transition-all space-y-2 group">
          <div class="flex items-center justify-between">
            ...
            <UBadge color="warning" variant="subtle" size="xs">{{ metrics.pendingPaymentCount }} Pending Payment</UBadge>
          </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
        ...
        >
          <div class="flex items-start gap-2.5">
            <span
              :class="[
                'size-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold',
                step.completed ? 'bg-emerald-500 text-white' : 'border-2 border-primary text-transparent'
              ]"
            >
              ✓
            </span>
      ...
      <!-- SELLER TIER RANK & UNLOCKABLE ACHIEVEMENT BADGES -- commented out for now
      <div class="p-5 rounded-2xl border border-default bg-background space-y-4 shadow-xs">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="p-5 bg-background border border-default rounded-xl space-y-2">
        ...
        <p class="text-[11px] text-dimmed">{{ store?.instagram_connected ? 'Meta Webhook live connected' : 'Instagram not connected yet' }}</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="space-y-3">
        ...
        <div class="border border-default rounded-xl bg-background divide-y divide-default">
          <div
            v-for="dm in recentConversations"
            :key="dm.id"
          ...
          <div v-if="recentConversations.length === 0" class="p-6 text-center text-xs text-dimmed">
    ...
    <!-- SHAREABLE INSTAGRAM STORY BADGE MODAL -- commented out for now (tied to achievements above)
